...
// DEMO QUICK ACCESS START
const demoQuickAccessButtons = document.querySelectorAll("[data-demo-login-code]");
// DEMO QUICK ACCESS END
// =====================================================
// Global State
// =====================================================
...
function getParentChildStatusText(hifzPercent, hifzMission, murMission) {
  if (hifzMission || murMission) {
    return "لديه مهام حالية";
  }
  if (hifzPercent >= 75) {
    return "متقدم بشكل ممتاز";
  }
  if (hifzPercent >= 30) {
    return "يتقدم بشكل جيد";
  }
  return "بحاجة متابعة";
}

function buildParentMissionCardHtml(title, description, toneClass) {
  return `
    <div class="parent-mission-card ${toneClass}">
      <div class="parent-mission-title">${title}</div>
      <div class="parent-mission-text">${description || "لا توجد مهمة حالية"}</div>
    </div>
  `;
}

function buildParentSummaryMetricCard(icon, label, value) {
  return `
    <div class="parent-summary-card">
      <span class="parent-summary-icon" aria-hidden="true">${icon}</span>
      <div class="parent-summary-label">${label}</div>
      <div class="parent-summary-value">${value}</div>
    </div>
  `;
}

function getOrderedExamPhaseIds() {
  if (!hasExternalCurriculumRuntime()) return [];

  const seen = new Set();
  return getExternalHifzCurriculumGroups()
    .map((group) => group?.phaseId || null)
    .filter((phaseId) => {
      if (!phaseId || seen.has(phaseId)) return false;
      seen.add(phaseId);
      const phase = getCurriculumPhase(phaseId);
      return Boolean(phase?.advance_requires_exam_pass);
    });
}

function countPassedHifzLevels(student = {}) {
  const orderedExamPhaseIds = getOrderedExamPhaseIds();
  if (!orderedExamPhaseIds.length) return 0;

  const targetPhaseId =
    String(student.hifz_pending_phase_id || "").trim() ||
    String(student.hifz_current_phase_id || "").trim() ||
    String(buildStudentCurriculumSnapshot(student)?.currentPhaseId || "").trim();

  if (!targetPhaseId) return 0;

  const targetIndex = orderedExamPhaseIds.indexOf(targetPhaseId);
  if (targetIndex === -1) return 0;
  return targetIndex;
}

function renderParentSummaryStrip(children = []) {
  if (!parentSummaryStrip) return;

  if (!children.length) {
    parentSummaryStrip.innerHTML = "";
    return;
  }

  const totalChildren = children.length;
  const averageProgress = Math.round(
    children.reduce((sum, student) => sum + computeHifzPercent(student), 0) / totalChildren
  );
  const totalPoints = children.reduce((sum, student) => sum + Number(student.total_points || 0), 0);
  const totalPassedLevels = children.reduce((sum, student) => sum + countPassedHifzLevels(student), 0);

  parentSummaryStrip.innerHTML = `
    ${buildParentSummaryMetricCard("👨‍👩‍👧", "عدد الأبناء", totalChildren)}
    ${buildParentSummaryMetricCard("📈", "متوسط الإنجاز", `${averageProgress}%`)}
    ${buildParentSummaryMetricCard("⭐", "إجمالي النقاط", totalPoints)}
    ${buildParentSummaryMetricCard("🏁", "المستويات المجتازة", totalPassedLevels)}
  `;
}
...
function createParentChildCard(student, ranksByHalaqa) {
  ...
  const statusText = getParentChildStatusText(hifzPercent, getCurrentHifzMission(student), getCurrentMurajaaMission(student));
  ...
  el.innerHTML = `
    <div class="child-card-head">
      <div>
        ...
        <div class="child-meta">${halaqaLabel} • ${student.code}</div>
      </div>
      <div class="child-status-badge">${statusText}</div>
    </div>
    <div class="child-progress-row">
      <div class="child-progress-copy">
        <div class="child-progress-label">إنجاز الحفظ</div>
        <div class="child-progress-value">${hifzPercent}%</div>
      </div>
      <div class="child-points-chip">النقاط: ${student.total_points || 0}</div>
    </div>
    <div class="progress-bar"><div class="progress-fill" style="width:${hifzPercent}%"></div></div>
    <div class="child-motivation">${motivation}</div>
    <div class="parent-missions-grid">
      ${buildParentMissionCardHtml("مهمة الحفظ", hifzMission ? hifzMission.description : "", "parent-mission-hifz")}
      ${buildParentMissionCardHtml("مهمة المراجعة", murMission ? murMission.description : "", "parent-mission-murajaa")}
    </div>
...
function buildParentWelcomeText(parentName, parentKey) {
  const normalizedParentName = String(parentName || "").trim();
  if (normalizedParentName) {
    return `مرحبًا ${normalizedParentName}`;
  }
...
function renderParentDashboardContent(children, ranksByHalaqa) {
  if (!children.length) {
    renderParentSummaryStrip([]);
    parentChildrenList.innerHTML = buildNoParentChildrenMessage();
    return;
  }

  renderParentSummaryStrip(children);
...
function buildParentDashboardData(allStudents, parentCode) {
  ...
  const parentName =
    children.find((student) => String(student.parent_name || "").trim())?.parent_name || "";
...
function prepareParentDashboardView(parentName, parentKey) {
  welcomeParent.textContent = buildParentWelcomeText(parentName, parentKey);
  if (parentSummaryStrip) parentSummaryStrip.innerHTML = "";
  parentChildrenList.innerHTML = "";
}

function renderParentDashboard({ parentKey, parentName, children, ranksByHalaqa }) {
  prepareParentDashboardView(parentName, parentKey);
...
async function displayParentDashboard(parentCode) {
  try {
    const { parentKey, parentName, children, ranksByHalaqa } = await buildParentDashboardContext(parentCode);
    renderParentDashboard({ parentKey, parentName, children, ranksByHalaqa });
  } catch (e) {
...
function showHalaqaScreen() {
  hideAllScreens();
  halaqaBackButton?.classList.toggle("hidden", isDirectHalaqaLink);
...
function extractDirectHalaqaLoginCode(pathname = window.location.pathname) {
  const searchParams = new URLSearchParams(window.location.search || "");
  const halaqaFromQuery = searchParams.get("halaqa");
  if (String(halaqaFromQuery || "").trim()) {
    return halaqaFromQuery.trim();
  }

  const normalizedPath = String(pathname || "").trim();
  const directPrefixMatch = normalizedPath.match(/^\/hal(?:a)?qa\/([^/]+)\/?$/i);
  if (!directPrefixMatch) return null;

  return decodeURIComponent(directPrefixMatch[1] || "").trim() || null;
}

async function resolveDirectHalaqaLinkContext(directLoginCode) {
  const loginContext = await resolveAppLoginContext(directLoginCode);
  if (loginContext?.kind === "halaqa") {
    return loginContext;
  }

  const fallbackMatch = String(directLoginCode || "").trim().toLowerCase().match(/^halaqa-(.+)$/);
  if (!fallbackMatch) {
    return loginContext;
  }

  const fallbackHalaqaCode = normalizeHalaqaCode(fallbackMatch[1]);
  const halaqas = await fetchAllHalaqasSnapshot(db);
  const matchedHalaqa = halaqas.find((halaqa) =>
    normalizeHalaqaCode(halaqa?.code || halaqa?.id || halaqa?.halaqa) === fallbackHalaqaCode
  );

  if (!matchedHalaqa?.code) {
    return loginContext;
  }

  return {
    kind: "halaqa",
    halaqaCode: String(directLoginCode).trim().toLowerCase(),
    halaqaType: matchedHalaqa.code,
  };
}
...
// DEMO QUICK ACCESS START
function showDemoQuickAccessError(message) {
  showMessage(authMessage, message, "error");
}

function clearDemoQuickAccessMessage() {
  authMessage?.classList.add("hidden");
  authMessage.textContent = "";
}

async function openDemoQuickAccessByCode(loginCode, expectedKind) {
  try {
    clearDemoQuickAccessMessage();
    const loginContext = await resolveAppLoginContext(loginCode);

    if (!loginContext || loginContext.kind !== expectedKind) {
      showDemoQuickAccessError("تعذر فتح الواجهة التجريبية المطلوبة حالياً.");
      return;
    }
    ...
    if (actionContext.loginKind === "halaqa") {
      await handleHalaqaLoginContext(loginContext);
      return;
    }

    if (actionContext.loginKind === "teacher") {
      await handleTeacherLoginContext(loginContext);
      return;
    }

    if (actionContext.loginKind === "director") {
      await handleDirectorLoginContext(loginContext);
      return;
    }

    showDemoQuickAccessError("هذا الاختصار غير مدعوم في العرض التجريبي الحالي.");
  } catch (error) {
    console.error("demo quick access error:", error);
    showDemoQuickAccessError("حدث خطأ أثناء فتح الاختصار التجريبي.");
  }
}

function bindDemoQuickAccessButtons() {
  demoQuickAccessButtons.forEach((button) => {
    if (button.dataset.bound === "true") return;

    button.addEventListener("click", () => {
      openDemoQuickAccessByCode(
        button.dataset.demoLoginCode || "",
        button.dataset.demoLoginKind || ""
      );
    });
...
bindDemoQuickAccessButtons();
// DEMO QUICK ACCESS END

async function initializeDirectHalaqaLink() {
  console.log("[direct-halaqa] search:", window.location.search);
  const directLoginCode = extractDirectHalaqaLoginCode();
  console.log("[direct-halaqa] code:", directLoginCode);
  if (!directLoginCode) return false;

  hideAllScreens();

  try {
    const loginContext = await resolveDirectHalaqaLinkContext(directLoginCode);
    console.log("[direct-halaqa] context:", loginContext);
    if (loginContext?.kind !== "halaqa") {
      goToAuthScreen({ clearInput: false });
      return false;
    }

    isDirectHalaqaLink = true;
    await openHalaqaDashboard(loginContext.halaqaCode, loginContext.halaqaType);
    return true;
  } catch (error) {
    console.error("initializeDirectHalaqaLink error:", error);
    goToAuthScreen({ clearInput: false });
    return false;
  }
}
