...
function getDirectorTabIcon(tabId) {
  const iconMap = {
    [DIRECTOR_TAB_IDS.OVERVIEW]: "🏠",
    [DIRECTOR_TAB_IDS.HALAQAS]: "🕌",
    [DIRECTOR_TAB_IDS.ACCOUNTS]: "👥",
    [DIRECTOR_TAB_IDS.REPORTS]: "📊",
    [DIRECTOR_TAB_IDS.SETTINGS]: "⚙️",
  };
...
function buildDirectorTabButtonsHtml(activeTabId = DIRECTOR_DEFAULT_TAB_ID) {
  return buildDirectorTabDefinitions()
    .map(
      (tab) => `
        <button
          class="tab-button director-top-tab${tab.id === activeTabId ? " active" : ""}"
          data-director-tab="${tab.id}"
        >
          <span class="director-top-tab-icon" aria-hidden="true">${getDirectorTabIcon(tab.id)}</span>
          <span class="director-top-tab-label">${getDirectorTabLabel(tab.label)}</span>
        </button>
      `
    )
    .join("");
...
function getDirectorSummaryCardIcon(title) {
  const iconMap = {
    "عدد الحلقات": "📚",
    "عدد الطلاب": "👨‍🎓",
    "عدد المعلمين": "👳",
...
function buildDirectorSummaryCard(title, value, options = {}) {
  const {
    toneClass = "",
    icon = getDirectorSummaryCardIcon(title),
  } = options;

  return `
    <div class="director-stat-card${toneClass ? ` ${toneClass}` : ""}">
      <div class="director-stat-topline">
        <span class="director-stat-icon" aria-hidden="true">${icon}</span>
...
function buildDirectorPendingTestsHtml(overview = {}) {
  ...
  return `
    <section class="tasks-section director-section">
      ...
      <div class="director-card-grid director-pending-tests-grid">
        ${items.map((item) => `
          <div class="task-card director-surface-card director-pending-test-card">
            <div class="task-header director-pending-test-header">
              <div>
                <div class="task-title">${item.name}</div>
                <div class="director-pending-test-code">${item.code}</div>
              </div>
              <span class="task-type-tag general">${item.halaqaLabel || "بدون حلقة"}</span>
            </div>
            <div class="action-buttons director-pending-test-actions">
              <div class="input-group director-pending-test-score-group">
  ...
  return `
    <section class="tasks-section director-section">
      ...
      <div class="director-stats-grid">
        ...
        ${buildDirectorSummaryCard("الحضور اليوم", dailySummary.presentTodayCount || 0, {
          toneClass: "director-stat-card-primary",
        })}
    ...
    <section class="tasks-section director-section">
      ...
      <div class="director-stats-grid">
        ${buildDirectorSummaryCard("الحاضرون اليوم", dailySummary.presentTodayCount || 0, {
          icon: "✅",
        })}
        ${buildDirectorSummaryCard("الغائبون اليوم", dailySummary.absentTodayCount || 0, {
          icon: "🚫",
        })}
        ${buildDirectorSummaryCard("المهام المعلقة", dailySummary.pendingTasksCount || 0, {
          icon: "⏳",
        })}
        ${buildDirectorSummaryCard("المهام المعتمدة", dailySummary.approvedTasksTodayCount || 0, {
          icon: "📌",
        })}
      </div>
    </section>
    ...
    .map(
      (student) => `
        <div class="task-card director-surface-card">
          ...
          <div class="action-buttons director-student-card-actions">
            <button
              type="button"
              class="director-status-toggle director-status-toggle-${student.statusValue === "inactive" ? "inactive" : "active"}"
              ...
              aria-pressed="${student.statusValue === "inactive" ? "false" : "true"}"
            >
              <span class="director-status-toggle-track">
                <span class="director-status-toggle-thumb" aria-hidden="true"></span>
                <span class="director-status-toggle-text">${student.statusValue === "inactive" ? "تشغيل الطالب" : "إيقاف الطالب"}</span>
              </span>
            </button>
            <button
              type="button"
...
function ensureDirectorScreen() {
  ...
  directorScreen.innerHTML = `
    ...
    <nav id="director-tabs" class="tabs director-top-nav director-surface-card"></nav>
