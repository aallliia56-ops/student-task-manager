...
function buildStudentTaskMetaHtml(pendingText = "") {
  if (!pendingText) return "";

  return `
    <div class="student-task-meta-row">
      <span class="student-task-status">${pendingText}</span>
    </div>
  `;
}

function createStudentPrimaryTaskCard({
  title,
  tagClass,
  description,
  points,
  ...
  card.className = `task-card ${tagClass} student-primary-task-card`;
  const visualIcon = tagClass === "hifz" ? "📖" : "↻";
  card.innerHTML = `
    <div class="student-task-shell">
      <div class="student-task-content">
        <div class="task-header student-task-header">
          <div class="task-title">${title}</div>
          <span class="task-points-tag student-task-points">النقاط: ${points}</span>
        </div>
        <div class="task-body mission-text student-task-body">
          ${description}
        </div>
        ${buildStudentTaskMetaHtml(pendingText)}
        <button type="button" class="button success student-task-action">${buttonText}</button>
      </div>
      <div class="student-task-visual" aria-hidden="true">
        <div class="student-task-visual-circle">${visualIcon}</div>
      </div>
    </div>
  `;

  const button = card.querySelector(".student-task-action");
...
export function renderStudentTasks(student, actions) {
  ...
  if (hifzMission) {
    if (hifzMission.mode === "flexible") {
      ...
      const card = createStudentPrimaryTaskCard({
        title: "مهمة الحفظ",
        tagClass: "hifz",
      ...
      if (!isAssistantPending) {
        const originalButton = card.querySelector(".student-task-action");
  ...
  for (const task of generalTasks) {
    ...
    card.innerHTML = `
      <div class="task-header student-task-header">
        ...
        <span class="task-points-tag student-task-points">النقاط: ${task.points}</span>
      </div>
      ...
      ${
        task.status === "assigned"
          ? ""
          : `<div class="student-task-meta-row">
        <span class="student-task-status">${
          task.status === "pending"
            ? "قيد المراجعة لدى المعلم..."
    ...
    btn.className = "button success student-task-action";
  ...
  if (!hifzPaused && isAwaitingHifzTest) {
    ...
    const card = document.createElement("div");
    card.className = "task-card hifz student-primary-task-card";
    card.innerHTML = `
      <div class="task-header student-task-header">
        <div class="task-title">مهمة الحفظ</div>
        <span class="task-points-tag student-task-points">النقاط: —</span>
      </div>
      <div class="task-body mission-text student-task-body">${pauseCopy.body}</div>
      <div class="student-task-meta-row">
        <span class="student-task-status">${pauseCopy.status}</span>
      </div>
    `;
    wrap.prepend(card);
  }
