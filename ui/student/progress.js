...
export function renderStudentProgress({ student, els, safeSetText, getStudentHifzPauseCardCopy }) {
  ...
  safeSetText(els.studentHifzNextLabel, `التالي: ${nextH ? (nextH.display_text || nextH.description) : "—"}`);
  safeSetText(els.studentMurajaaNextLabel, `التالي: ${nextM ? nextM.description : "—"}`);
